...
if(isset($_POST['submit'])) {
    ...
    if(empty($errors)) {
        $query = mysqli_query($con, "INSERT INTO `courses_session`(`course_ID`, `start_date`, `end_date`) VALUES ('$courseName','$startDate','$endDate')");
        
        if ($query) {
            $alertStyle ="alert alert-success";
            $statusMsg="Course Session Added Successfully!";
            $courseName=$startDate=$endDate="";
            header("refresh:2");
        }
        else {
            $alertStyle ="alert alert-danger";
            $statusMsg="An error occurred while adding the Course Session.";
        }
    } else {
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <div id="pay-invoice">
                                    <div class="card-body">
                                    <div class="<?php echo $alertStyle;?>" role="alert"><?php echo $statusMsg;?></div>                                        <form method="Post" action="">
                                        <form method="Post" action="">
                                            <div class="row">
                                                ...
                                                <div class="col-6">
                                                        ...
                                                        <?php
                                                            $result=mysqli_query($con,"SELECT `course_ID`, `course_name` FROM `courses`");
                                                            while ($row=mysqli_fetch_array($result))
                                                            {
                                                        ?>
                                                            
                                                         
                                                            <option value="<?=$row['course_ID']?>"  <?php if($row['course_ID'] == $courseName) { echo 'selected="selected"'; } ?> ><?=$row['course_name']?></option>
                                                            <?php
                                                            }
                                                            ?>
                                                            </select>  
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <table id="bootstrap-data-table" class="table table-hover table-striped table-bordered">
                                    ...
                                    <tbody>
                                    <?php
                                $result=mysqli_query($con,"SELECT cs.session_ID, c.course_name , cs.start_date, cs.end_date
