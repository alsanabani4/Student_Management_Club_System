...
$firstname = $lastname = $dob = $address = $role = $username = $phone = $gender = "";
if(isset($_GET['id']))
{
    $id = (int) mysqli_real_escape_string($con, $_GET['id']);
    $id = is_numeric($id) ? $id : NULL;
    $query1 = "SELECT * FROM `employees` WHERE `employee_ID` = $id";
...
if(isset($_POST['submit'])){
    ...
    if(empty($errors)) {
        $query = mysqli_query($con, "UPDATE `employees` SET `first_name`='$firstname',`last_name`='$lastname',`gender`='$gender',`address`='$address',`phone`='$phone',`birthdate`='$dob',`role`='$role',`username`='$username' WHERE employee_ID=$id");
        ...
        if ($query) {
            ...
            $statusMsg="Employee Updated Successfully!";
             ...
             $firstname = $lastname = $dob = $address = $role = $username  = $phone = $gender = "";
             header("refresh:2; url=viewEmployee.php");
        } else {
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <strong class="card-title"><h2 align="center">Update Employee</h2></strong>
                    </div>
                    <div class="card-body">
                        <!-- Credit Card -->
                        <div id="pay-invoice">
                            <div class="card-body">
                                    ...
                                    <div class="row">
                                        <div class="col-6">
                                            <div class="form-group">
                                                ...
                                                <input id="" name="dob" type="date" class="form-control cc-exp" value="<?php echo $dob; ?>" Required >
                                            </div>
                                        </div>
                                        <div class="col-6">
                                ...
                                <button type="submit" name="submit" class="btn btn-success">Update Employee</button>
                            </form>
                        </div>
                    </div>
                </div> <!-- .card -->
